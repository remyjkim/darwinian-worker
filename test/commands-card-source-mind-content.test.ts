// ABOUTME: Verifies persona, beliefs, and memory source authoring through `drwn card source`.
...
test("mind content source commands scaffold files and append explicit visibility", async () => {
  ...
  const persona = await runAgentsCli(["card", "source", "add-persona", "@me/example", "voice", "--visibility", "internal"], envFor(fixture));
  const belief = await runAgentsCli(["card", "source", "add-belief", "@me/example", "engineering", "--visibility", "public"], envFor(fixture));
  const memory = await runAgentsCli(
    ["card", "source", "add-memory", "@me/example", "transcripts", "--layer", "l6", "--visibility", "private", "--format", "jsonl"],
    envFor(fixture),
  );
  ...
  expect(existsSync(join(sourceDir(fixture), "memory", "l6", "transcripts", "memory.jsonl"))).toBe(true);
  ...
  expect(manifest.memory.l6).toEqual({ include: ["transcripts"], visibility: "private", format: "jsonl" });
});

test("remove-persona supports dry-run, deletion, and keep-files", async () => {
  const fixture = await scaffoldSourceFixture();
  expect((await runAgentsCli(["card", "source", "add-persona", "@me/example", "voice", "--visibility", "internal"], envFor(fixture))).exitCode).toBe(0);
  ...
  expect(JSON.parse(dryRun.stdout).changes.map((change: { action: string }) => change.action))
    .toEqual(["remove-persona-files", "update-manifest"]);
...
test("card source doctor reports mind content directory issues and l6 size warnings", async () => {
  const fixture = await scaffoldSourceFixture();
  const manifest = await readManifest(fixture);
  manifest.persona = { include: ["voice", "missing"], visibility: "internal" };
  manifest.beliefs = { include: ["engineering"], visibility: "public" };
  manifest.memory = { l6: { include: ["raw"], visibility: "private", format: "jsonl" } };
  ...
  await mkdir(join(sourceDir(fixture), "memory", "l6", "raw"), { recursive: true });
  await writeFile(join(sourceDir(fixture), "memory", "l6", "raw", "memory.jsonl"), `${JSON.stringify({ large: "x".repeat(1024 * 1024 + 1) })}\n`);
  ...
  expect(parsed.issues.map((issue: { code: string; severity: string }) => `${issue.severity}:${issue.code}`)).toEqual(
    expect.arrayContaining([
      ...
      "warning:memory_l6_size_warning",
    ]),
  );
});
...
test("mind content mutations honor DRWN_STORE_READONLY while dry-run still reports plans", async () => {
  ...
  const blockedAdd = await runAgentsCli(["card", "source", "add-persona", "@me/example", "voice", "--visibility", "internal"], readonlyEnv);
  ...
  const dryRunRemove = await runAgentsCli(["card", "source", "remove-persona", "@me/example", "voice", "--dry-run", "--json"], readonlyEnv);

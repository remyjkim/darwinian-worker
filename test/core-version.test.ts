...
import { HOOKS_MIN_DRWN_VERSION, MINDS_MIN_DRWN_VERSION } from "../cli/core/card-lock";
...
describe("drwn version reconciliation", () => {
  ...
  test("running version is at least the highest floor drwn can emit", () => {
    expect(gte(DRWN_VERSION, MINDS_MIN_DRWN_VERSION)).toBe(true);
    expect(gte(MINDS_MIN_DRWN_VERSION, HOOKS_MIN_DRWN_VERSION)).toBe(true);
  });
});

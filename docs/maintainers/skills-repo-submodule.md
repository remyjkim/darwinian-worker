...
This repo embeds `darwinian-harness-skills` as a shallow git submodule at `darwinian-harness-skills/`. This page is the contributor-side reference for that relationship.

## Why it exists

The skills repo at [`github.com/remyjkim/darwinian-harness-skills`](https://github.com/remyjkim/darwinian-harness-skills) is the canonical authoring source for the broader skill catalog and harness cards that the `drwn` CLI consumes (`drwn library add skill`, `drwn add card`, the package-backed bundle flow). It is a separately published artifact — distributed via the Claude Code plugin marketplace, Codex, Vercel `npx skills add`, and as a `drwn` bundle — but lives in lockstep with this CLI repo.

Adding it as a submodule means a contributor's `git clone --recurse-submodules` of `darwinian-harness` gives them both checkouts in one tree, with no separate sibling-repo setup required. It also makes the skill content discoverable from inside this repo without forcing every contributor to remember a second URL.
...
- `darwinian-harness-skills/` — submodule working tree (a Git gitlink in the parent repo's index)
...
```bash
cd darwinian-harness-skills
git pull origin main
cd ..
git add darwinian-harness-skills
git commit -m "[chore:submodule] bump darwinian-harness-skills to <short-sha>"
git push
```

Notes:

- The submodule tracks upstream `main` by default. To pin to a specific tag instead, `cd darwinian-harness-skills && git checkout <tag>` before staging the gitlink.
...
- To get full history locally (e.g. to author a skills-repo change): `cd darwinian-harness-skills && git fetch --unshallow`.
...
```bash
npm pack --dry-run --json | jq '.[0].files[].path' | grep darwinian-harness-skills
```
...
CI configurations that clone this repo without `--recurse-submodules` (or without a separate `git submodule update --init` step) will see `darwinian-harness-skills/` as an empty directory. Nothing in this repo's build, test, or typecheck pipeline depends on the submodule contents:
...
```bash
git submodule deinit -f darwinian-harness-skills
git rm -f darwinian-harness-skills
rm -rf .git/modules/darwinian-harness-skills
git commit -m "[chore:repo] remove darwinian-harness-skills submodule"
```

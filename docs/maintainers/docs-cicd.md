...
- The submodule (`darwinian-harness-skills/`) is **not** fetched in CI (`submodules: false` in every workflow). The docs site is self-contained; pulling the submodule would add clone time for no build benefit.

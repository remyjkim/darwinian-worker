...
Card search reads `catalog.json` entries inside catalogs registered via `drwn library catalog add` (the default community catalog is registered by `drwn init` unless `--no-default-catalogs` is passed).

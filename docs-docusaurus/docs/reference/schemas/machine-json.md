...
For `defaults.skills` and `defaults.mcpServers`, only a non-empty array is treated as an explicit machine override. A missing field or an empty array (`[]`) means "uninitialized": reads fall back to the resolved defaults, and `drwn library defaults add|remove ...` seeds the list with the currently resolved defaults before applying the mutation.

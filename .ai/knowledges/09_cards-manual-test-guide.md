...
```bash
drwn library defaults add skill machine-only
...
The captured source contains only the selected closure plus explicit project overlays. It excludes alternative roots, machine defaults, ambient user-home capabilities, generated bytes, platform connectors, and secret values.
...
drwn doctor --json
drwn store export --output "$DRWN_SANDBOX/unsafe.tar"
```
...
- whole-Store export fails with `STORE_EXPORT_DISABLED_UNSAFE`;

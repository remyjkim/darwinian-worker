...
- machine Store: `AGENTS_DIR/drwn`, normally `~/.agents/drwn`;
...
The Store contains inventory, machine intent, caches, generated machine output, registrations, and credentials. These categories do not all share one portability or ownership policy. Whole-Store export is disabled.
...
- machine defaults and machine Library activation are not project declarations.
...
Ambient observations are diagnostic-only in Task 77. Task 83 owns target-specific collision classification and selected-target write preflight.
...
## Machine And Library Boundaries

Machine defaults remain a separate activation authority for explicit machine writes. They are not prepended to project capability state.

Task 80 will define a clean namespaced machine schema and an optional Recommended Darwinian Operator guided profile. That future profile may project machine-safe skills and approved MCP definitions only; it does not create a project Worker or project declaration.

Library inventory is reusable standalone skill/MCP content. Cards own their bundled content. Complete Library lifecycle policy remains Task 81 work; portable inventory transfer remains Task 82 work.
...
`card new --from-project` captures the selected closure and explicit project overlays. It excludes inactive alternatives, machine defaults, ambient user-home state, generated bytes, platform connectors, and resolved secrets. No selected root fails without creating a source.

## Store Safety

`drwn store export` is fail-closed with `STORE_EXPORT_DISABLED_UNSAFE`; there is no broad override. Credentials, machine intent, registrations, write records, and caches therefore cannot be accidentally archived by that command.
...
- Integration: project transactions, Store resolution, target adapters.

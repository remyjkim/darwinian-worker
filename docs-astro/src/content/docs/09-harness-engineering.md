...
## Where darwinian Fits

`darwinian` occupies the local operator layer of harness engineering. Its job is to make scattered local agent configuration explicit and governable:
...
This is why the name is `darwinian` — it is the starting harness layer around every local agent setup, not a replacement for any one agent.
